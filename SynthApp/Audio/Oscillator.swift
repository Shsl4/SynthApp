...
public class Oscillator {
    ...
    public init(sampleRate: Float) {
        ...
        self.filter = CombFilter(sampleRate: sampleRate, frequency: 500.0, q: 1.0)//LowPassFilter(sampleRate: sampleRate, frequency: 500.0, q: 1.0)

        for voiceID in 0 ..< 8 {
            addVoice(voice: Voice(sampleRate: sampleRate, voiceID: voiceID, envelopeReleased: onNoteStopped))
        }
    ...
    public func getVoicePlayingNote(noteNumber: Int) -> Voice? {
        ...
        for voice in activeVoices where voice.note == noteNumber {
    ...
    public func setBypassed(_ bypass: Bool) {
    ...
    public func onNoteOn(noteNumber: Int, velocity: Int) {
    ...
    public func fillBuffer(_ bufferList: UnsafeMutableAudioBufferListPointer, _ frameCount: AUAudioFrameCount, _ bufferOffset: AUAudioFrameCount) {
